              ...
              case "tool-convertCurrency":
                ...
                switch (part.state) {
                  case "input-streaming":
                    return (
                      <div
                        key={`${message.id}-convertCurrency-${index}`}
                        className="bg-zinc-800/50 border border-zinc-700 p-2 rounded mt-1 mb-2"
                      >
                        <div className="text-sm text-zinc-500">💱 Receiving conversion request…</div>
                    ...
                    return (
                      ...
                      >
                        <div className="text-sm text-zinc-400">
                          💱 Converting {part.input.amount} {String(part.input.from).toUpperCase()} →{" "}
                          {String(part.input.to).toUpperCase()}…
                        </div>
                      </div>
                    );
                  ...
                  case "output-available":
                    return (
                      <div key={`${message.id}-convertCurrency-${index}`} className="mt-1 mb-2">
                        <div className="text-sm text-zinc-400">💱 Conversion</div>
                        <CurrencyCard data={part.output} />
                      </div>
                    );
      ...
      >
        <div className="flex gap-2">
          <input
            ...
            placeholder='Try:"Convert 100 EUR to USD" or "20,000 JPY to EUR"'
          />
          {status === "submitted" || status === "streaming" ? (
            <button
              onClick={stop}
